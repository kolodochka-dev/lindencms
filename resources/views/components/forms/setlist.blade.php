...
<x-cms::forms.group :label="$label" :required="$required" :hidden="$hidden" :for="$input_id">
    <div class="setlist droplist">
        <div class="border border-base-300 rounded-md p-2 text-xs flex justify-between w-full">
            <div class="container w-full flex flex-wrap items-center gap-2">
                {{-- pills... --}}
        ...
        <div class="droplist-select w-full">
            <div class="bg-base-100 z-1 w-full p-2 border  rounded-md max-h-60 overflow-y-auto">
                <input
                    {{ $attributes->merge([
                        'class' => 'search py-1.5 px-3 w-full border border-gray-200 rounded-lg text-xs focus:border-secondary outline-none disabled:opacity-50 disabled:pointer-events-none',
                        'id' => $input_id,
                        'disabled' => $disabled,
                ...
                <div class="options flex flex-col gap-1 ">
                    @foreach ($options as $option)
                        <label class="label text-xs text-base-content hover:bg-slate-100 rounded px-1 py-1">
                            <input type="radio" name="{{ $name }}" value="{{ $option->get() }}"
                                class="radio radio-xs" {{ old($dot_name, $value) == $option->get() ? 'checked' : '' }}
                                autocomplete="off" 
                            />
                            <div class="pill-templ">
                                {{ $option->render('plain') }}
                            </div>
                        </label>
                    @endforeach
